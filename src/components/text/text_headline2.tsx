...
const TextHeadline2: React.FC<Props> = ({ title, color, fontweight, fontFamily, bottom, right, textAlign }) => {
  return (
    <Text style={{ fontSize: SizeConfig.head2, color: color ?? colors.dark, fontWeight: fontweight ?? 'normal', fontFamily: fontFamily, bottom: bottom, right: right, textAlign: textAlign ?? 'left' }} >{title}</Text>
  );
}
